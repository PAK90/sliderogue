...
function ShopDialog() {
  const { shopping, closeShopping, score, applyUpgrade } = useGameStore();

  const upgradeDisplay = (upgrade: Upgrade) => {
    const disabled = (upgrade?.cost || 0) > score;
    return (
  ...
  return (
    <>
      ...
      >
        {`You have ${score} points to spend`}
